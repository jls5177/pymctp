...
def main(args) -> int:
    ...
    #     resp = ovl1.session.sndrcv_control_msg(DiscoveryNotify(), dst_eid=0x0a,
    ...
    if args.send_discovery_notify:
        print(f"Sending DiscoveryNotify ...")
        resp = ovl1.session.sndrcv_control_msg(DiscoveryNotify(), dst_eid=0x0A, dst_phy_addr=bmc_addr, timeout_s=0.3)
        if resp:
            print(f"DiscoveryNotify response: ")
            resp.show2()
    ...
    #                                     dst_eid=0x0A, dst_phy_addr=bmc_addr, timeout_s=5, threaded=True)
    # if resp:
    ...
    #        GetDeviceEidRequestPacket(vendor_id=0x1414, device_id=0x7, subsystem_vendor_id=0x1414,
    #                                  subsystem_device_id=0x7, instance=0))
    # resp = hsp1.session.sndrcv_mctp_msg(req, msg_type=MsgTypes.VDPCI, msg_tag=0x3,
    #                                     dst_eid=0x0A, dst_phy_addr=bmc_addr, timeout_s=5, threaded=True)
    # if resp:
    #     print(f"GetDeviceEID response: \n\t{resp.summary()}")
    ...
    #                                  subsystem_device_id=0x3, instance=0))
    # resp = hsp1.session.sndrcv_mctp_msg(req, msg_type=MsgTypes.VDPCI, msg_tag=0x3,
    #                                     dst_eid=0x0A, dst_phy_addr=bmc_addr, timeout_s=5, threaded=True)
    # if resp:
    #     print(f"GetDeviceEID response: \n\t{resp.summary()}")
