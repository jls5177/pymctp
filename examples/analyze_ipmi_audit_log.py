...
# need to keep the following import to bind the MSFT specific Packets
# import pymctp.oem.microsoft  # noqa
    ...
    for line in iter(monitor_fd.readline, ""):
        ...
        for timestamp, cmd in parsed_data.items():
            ...
            if ipmi_packet.is_request():
                ...
                rqt_pkt = ipmi_packet
                continue
            elif not req_timestamp:
                continue

            elapsed_time: timedelta = timestamp - req_timestamp
            elapsed_time_ms = elapsed_time.total_seconds() * 1000
            if elapsed_time_ms > 100.0:
                print(f"Elapsed time: {elapsed_time}, {elapsed_time_s}, {elapsed_time.microseconds / 1000} ms")
                req_pkt_summary = f"{req_timestamp.isoformat()}: {rqt_pkt.summary()}"
                print(req_pkt_summary)
