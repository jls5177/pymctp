...
bind_layers(VdPciHdrPacket, ErrorResponsePacket, vendor_id=VdPCIVendorIds.Msft,
...
bind_layers(VdPciHdrPacket, FwVersionCmdPacket, vendor_id=VdPCIVendorIds.Msft,
...
class DeviceCapsRequestPacket(AllowRawSummary, Packet):
    name = "Dev-Caps REQ"
    fields_desc = [XLEShortField("max_message", 0), XLEShortField("max_packet", 0), XByteField("device_info", 0),
                   XByteField("features", 0), XByteField("pk_key_strength", 0), XByteField("enc_key_strength", 0), ]

    def mysummary(self) -> Union[str, Tuple[str, List[AnyPacketType]]]:
        summary = (f"{self.name} (max_msg: {self.max_message}, max_pkt: {self.max_packet}, "
...
class DeviceCapsResponsePacket(AllowRawSummary, Packet):
    ...
    fields_desc = [XLEShortField("max_message", 0), XLEShortField("max_packet", 0), XByteField("device_info", 0),
                   XByteField("features", 0), XByteField("pk_key_strength", 0), XByteField("enc_key_strength", 0),
                   XByteField("message_timeout", 0), XByteField("crypto_timeout", 0), ]
...
bind_layers(VdPciHdrPacket, DeviceCapsCmdPacket, vendor_id=VdPCIVendorIds.Msft,
...
class GetLogResponsePacket(AllowRawSummary, Packet):
    ...
    def mysummary(self) -> Union[str, Tuple[str, List[AnyPacketType]]]:
        summary = (f"{self.name} (debug: 0x{self.debug_log_length:02X}, "
...
bind_layers(VdPciHdrPacket, GetLogInfoCmdPacket, vendor_id=VdPCIVendorIds.Msft,
...
class ReadLogRequestPacket(AllowRawSummary, Packet):
    ...
    def mysummary(self) -> Union[str, Tuple[str, List[AnyPacketType]]]:
        summary = (f"{self.name} (type: {self.log_type}, "
                   f"offset: 0x{self.offset:02X})")
        return summary, [VdPciHdrPacket]
...
bind_layers(VdPciHdrPacket, ReadLogCmdPacket, vendor_id=VdPCIVendorIds.Msft,
...
class ExtendedUpdateStatusRequestPacket(AllowRawSummary, Packet):
    ...
    fields_desc = [XByteField("update_type", 0), XByteField("port_id", 0), ]
...
class ExtendedUpdateStatusResponsePacket(AllowRawSummary, Packet):
    ...
    fields_desc = [XLEIntField("status", 0), XLEIntField("remaining_bytes", 0), ]
...
bind_layers(VdPciHdrPacket, ExtendedUpdateCmdPacket, vendor_id=VdPCIVendorIds.Msft,
            vdm_cmd_code=ChallengeCmdCodes.CERBERUS_PROTOCOL_GET_EXT_UPDATE_STATUS.value)


def _get_update_type_name(cmd_code: int) -> str:
    if (ChallengeCmdCodes.CERBERUS_PROTOCOL_INIT_FW_UPDATE.value
            <= cmd_code <= ChallengeCmdCodes.CERBERUS_PROTOCOL_COMPLETE_FW_UPDATE.value):
        return "FW"
    if (ChallengeCmdCodes.CERBERUS_PROTOCOL_INIT_PCD_UPDATE.value
            <= cmd_code <= ChallengeCmdCodes.CERBERUS_PROTOCOL_COMPLETE_PCD_UPDATE.value):
        return "PCD"
    if (ChallengeCmdCodes.CERBERUS_PROTOCOL_INIT_PFM_UPDATE.value
            <= cmd_code <= ChallengeCmdCodes.CERBERUS_PROTOCOL_COMPLETE_PFM_UPDATE.value):
        return "PFM"
    if (ChallengeCmdCodes.CERBERUS_PROTOCOL_INIT_CFM_UPDATE.value
            <= cmd_code <= ChallengeCmdCodes.CERBERUS_PROTOCOL_COMPLETE_CFM_UPDATE.value):
        return "CFM"
...
bind_layers(VdPciHdrPacket, PrepareUpdatePacket, vendor_id=VdPCIVendorIds.Msft,
...
bind_layers(VdPciHdrPacket, GenericUpdatePacket, vendor_id=VdPCIVendorIds.Msft,
...
class ActivateUpdatePacket(AllowRawSummary, Packet):
    ...
    fields_desc = [
        ...
        ConditionalField(
            ...
            lambda pkt: pkt.underlayer.getfieldval('vdm_cmd_code') in [
...
bind_layers(VdPciHdrPacket, ActivateUpdatePacket, vendor_id=VdPCIVendorIds.Msft,
