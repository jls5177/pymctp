...
class GetMctpVersionSupportPacket(Packet):
    ...
    fields_desc = set_control_fields(
        ...
        rsp_fields=[
            FieldLenField('version_number_entry_count', None, fmt='!B',
                          count_of='version_number_list'),
            FieldListField('version_number_list', [], XLEIntField('', 0),
...
def GetMctpVersionSupportResponse(_pkt: bytes | bytearray = b"", /, *,
    ...
    if _pkt:
        return GetMctpVersionSupportPacket(_pkt, _underlayer=hdr)
    return GetMctpVersionSupportPacket(msg_type_cnt=len(version_number_list),
